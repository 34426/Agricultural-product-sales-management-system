<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="退货地址" prop="tuihuodizhi">
          <el-input v-model="ruleForm.tuihuodizhi" 
              placeholder="退货地址" clearable  :readonly="ro.tuihuodizhi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="退货地址" prop="tuihuodizhi">
              <el-input v-model="ruleForm.tuihuodizhi" 
                placeholder="退货地址" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  data() {
	...
	money : false,
      ...
      rules: {
          dianjiaming: [
                { required: true, message: '店家名不能为空', trigger: 'blur' },
          ],
          mima: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhaopian = true;
          ...
          if(o=='tuihuodizhi'){
            this.ruleForm.tuihuodizhi = obj[o];
	    this.ro.tuihuodizhi = true;
            continue;
          }
          if(o=='money'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `dianjia/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.dianjiaCrossAddOrUpdateFlag = false;
                  this.parent.search();
