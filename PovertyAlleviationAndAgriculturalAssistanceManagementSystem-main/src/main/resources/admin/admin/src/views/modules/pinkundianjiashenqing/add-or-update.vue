<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="申请资料" prop="shenqingziliao">
          <file-upload
          tip="点击上传申请资料"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.shenqingziliao" label="申请资料" prop="shenqingziliao">
            <el-button type="text" size="small" @click="download(ruleForm.shenqingziliao)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="申请说明" prop="shenqingshuoming">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.shenqingshuoming" label="申请说明" prop="shenqingshuoming">
                    <span>{{ruleForm.shenqingshuoming}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.dianjiaming!=''&&json.dianjiaming){
              		this.ruleForm.dianjiaming = json.dianjiaming
		}
    ...
    info(id) {
      this.$http({
        url: `pinkundianjiashenqing/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `pinkundianjiashenqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.pinkundianjiashenqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
